...
class LELSD:
    ...
    # works as well for stylegan3-t, while it is not for trgb
    stylegan2_s_dims = {
        0: 512,  # 4 x 4 s1
        1: 512,  # 4 x 4 trgb
        2: 512,  # 8 x 8 s1
    ...
    def __init__(self, device, localization_layers, semantic_parts, loss_function="MSE",
                 ...
                 log_dir='log', random_seed=1234, sg='sg2'):
        ...
        if self.latent_space.startswith("S"):
            ...
            self.latent_dirs = []
            if sg != "sg2":
                for layer in self.stylegan3_s_dims:
                    dim = self.stylegan3_s_dims[layer]
