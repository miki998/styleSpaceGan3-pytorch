...
@st.cache(ttl=None, allow_output_mutation=True, max_entries=1,
          hash_funcs={
              ...
              StyleGAN3SampleGenerator: lambda s: hash(s.truncation_psi),
          })
...
# except:
#     base_lelsd = LELSD.load(base_lelsd_path, map_location=torch.device('cpu'))
...
if model_name == 'stylegan1':
    ...
    sample_generator = StyleGAN3SampleGenerator(G=G, device=device, truncation_psi=truncation_psi)
else:
    pass
